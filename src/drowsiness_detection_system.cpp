...
namespace DrowsinessDetector
{
    ...
    //     // 5. Display logic - choose between full frame or ROI view
    //     if (config_.show_full_frame)
    //     {
    ...
    //         // Update face tracking using original coordinates
    //         updateFaceTracking(display_face_rect, scale_factor);

    //         // Log with original frame if needed
    //         if (current_state != DriverState::ALERT)
    //         {
    //             std::string message = generateStateMessage(current_state);
    //             Logger::log(current_state, message, avg_ear, mar, frame);
    ...
    //     else
    //     {
    //         // Default: Show ROI frame (simpler, faster)
    //         drawVisualization(roi_frame, face_rect, current_state, avg_ear, mar);
    //         cv::imshow("Drowsiness Detection System", roi_frame);

    //         // Update face tracking directly (coordinates already in processed frame space)
    ...
    //     // Optional: Show both frames side by side for debugging
    //     if (config_.show_debug_view)
    //     {
